...
- push 到 `main` 后通过 GitHub Actions 自动部署
...
主要内容：
- `data/sources/manifest.json`：来源白名单与来源元数据
...
作用：把原始数据加工成站点实际展示所需的 JSON。

主要脚本：
- `validate-data.mjs`：校验数据完整性、素材存在性与发布门槛
- `fetch-sources.mjs`：保守版自动抓取入口，会对 approved/active 来源做抓取并自动更新 slow-variable signals，同时生成抓取报告
- `build-evidence.mjs`：生成语料快照
- `compute-clock.mjs`：计算当前时钟状态、证据流、历史记录、元数据
